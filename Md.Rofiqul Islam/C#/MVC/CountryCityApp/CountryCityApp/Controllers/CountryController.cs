...
namespace CountryCityApp.Controllers
{
    public class CountryController : Controller
    {
        ...
        public ActionResult Search(string CountrySearch, string SearchText, int? CountrySearchSelected)
        {
            List<Country>acList=new List<Country>();
            if(CountrySearch=="CountryName")
            {
                acList = db.Countries.Where(c => c.Name.Contains(SearchText)).ToList();

            }
            else if (CountrySearch == "CountryList")
            {
                acList = db.Countries.Where(c => c.CountryId == CountrySearchSelected).ToList();

            }


            ViewBag.CountrySearchSelected=new SelectList(db.Countries,"CountryId","Name");
